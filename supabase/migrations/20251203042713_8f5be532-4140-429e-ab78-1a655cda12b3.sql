-- Fix check_tenant_limits to count DISTINCT users instead of roles
...
BEGIN
  ...
  v_aggregated := get_tenant_aggregated_limits(p_tenant_id);
  
  -- Si no hay suscripción activa
  IF (v_aggregated->>'active_subscriptions_count')::INTEGER = 0 THEN
  ...
  CASE p_resource_type
    WHEN 'user' THEN
      v_limit := (v_aggregated->>'max_users')::INTEGER;
      -- FIXED: usar COUNT(DISTINCT user_id) para contar usuarios únicos
      -- Un usuario con múltiples roles (ej: PROPIETARIO + INQUILINO) cuenta como 1
    ...
    WHEN 'property' THEN
      v_limit := (v_aggregated->>'max_properties')::INTEGER;
      SELECT COUNT(*) INTO v_current_count
      FROM pms_properties
      WHERE tenant_id = p_tenant_id;
      
    WHEN 'contract' THEN
      v_limit := (v_aggregated->>'max_contracts')::INTEGER;
      SELECT COUNT(*) INTO v_current_count
      FROM pms_contracts
      WHERE tenant_id = p_tenant_id
    ...
    WHEN 'branch' THEN
      v_limit := (v_aggregated->>'max_branches')::INTEGER;
      SELECT COUNT(*) INTO v_current_count
      FROM pms_tenants
