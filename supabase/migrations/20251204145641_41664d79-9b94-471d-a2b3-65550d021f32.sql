-- 1. Add change_type column to subscription_change_requests
ALTER TABLE subscription_change_requests 
ADD COLUMN IF NOT EXISTS change_type TEXT DEFAULT 'replacement' 
CHECK (change_type IN ('replacement', 'addon', 'remove_addon'));

-- 2. Add addon_subscription_id for remove_addon requests
ALTER TABLE subscription_change_requests 
ADD COLUMN IF NOT EXISTS addon_subscription_id UUID REFERENCES tenant_subscriptions(id);

-- 3. Fix check_tenant_limits to count only ACTIVE properties (status != 'inactive')
...
BEGIN
  ...
  CASE p_resource_type
    ...
    WHEN 'property' THEN
      ...
      -- FIXED: Contar solo propiedades activas (status != 'inactive')
...
-- 4. Create helper function to simulate limits after removing a subscription
CREATE OR REPLACE FUNCTION public.simulate_limits_without_subscription(
  p_tenant_id UUID,
...
BEGIN
  RETURN QUERY
  SELECT
    MAX(COALESCE(sp.max_users, 9999)) AS total_max_users,
    SUM(COALESCE(sp.max_properties, 9999))::INTEGER AS total_max_properties,
    SUM(COALESCE(sp.max_contracts, 9999))::INTEGER AS total_max_contracts,
    SUM(COALESCE(sp.max_branches, 9999))::INTEGER AS total_max_branches
  FROM tenant_subscriptions ts
  JOIN subscription_plans sp ON sp.id = ts.plan_id
  WHERE ts.tenant_id = p_tenant_id
    AND ts.status IN ('active', 'trial')
    AND ts.id != p_subscription_id;
END;
$$;