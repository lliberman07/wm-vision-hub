-- =====================================================================
-- MIGRACIÓN 3: RPC de aplicación de ajustes y restructuración de tabla adjustments
-- =====================================================================

-- 1. Renombrar tabla existente como backup
ALTER TABLE IF EXISTS pms_contract_adjustments RENAME TO pms_contract_adjustments_backup;

-- 2. Crear nueva tabla con estructura completa
CREATE TABLE pms_contract_adjustments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  contract_id uuid NOT NULL REFERENCES pms_contracts(id) ON DELETE CASCADE,
  period_from date NOT NULL,
  period_to date NOT NULL,
  factor numeric NOT NULL,
  pct_cumulative numeric NOT NULL,
  prev_amount numeric NOT NULL,
  new_amount numeric NOT NULL,
  item_a_prev_amount numeric NULL,
  item_a_new_amount numeric NULL,
  item_b_prev_amount numeric NULL,
  item_b_new_amount numeric NULL,
  applied_at date NOT NULL,
  created_at timestamptz DEFAULT now(),
  tenant_id uuid NOT NULL REFERENCES pms_tenants(id),
  UNIQUE (contract_id, applied_at)
);
...
CREATE POLICY "Staff can manage contract adjustments"
  ON pms_contract_adjustments FOR ALL
  USING (
...
CREATE POLICY "Owners can view adjustments of their contracts"
...
-- 4. RPC principal de aplicación de ajuste
...
-- 5. Permisos
GRANT EXECUTE ON FUNCTION rpc_apply_contract_adjustment(uuid, date) TO authenticated;