-- Fix billing_cycle enum values: change "annual" to "yearly"
...
BEGIN
  ...
  -- FIXED: yearly instead of annual
  SELECT COALESCE(SUM(
    CASE 
...
-- Fix get_granada_subscription_analytics: yearly instead of annual
  ...
  v_result := jsonb_build_object(
    ...
    'billing_distribution', (
      SELECT jsonb_build_object(
        ...
        'ars_yearly', COALESCE(COUNT(*) FILTER (WHERE billing_cycle = 'yearly' AND currency = 'ARS'), 0),
        'usd_monthly', COALESCE(COUNT(*) FILTER (WHERE billing_cycle = 'monthly' AND currency = 'USD'), 0),
        'usd_yearly', COALESCE(COUNT(*) FILTER (WHERE billing_cycle = 'yearly' AND currency = 'USD'), 0)
      )
      FROM tenant_subscriptions
