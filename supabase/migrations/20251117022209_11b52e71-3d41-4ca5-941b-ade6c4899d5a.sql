-- Fix analytics functions: remove currency filter that doesn't exist
  ...
  v_result := jsonb_build_object(
    ...
    'billing_distribution', (
      SELECT jsonb_build_object(
        'monthly', COALESCE(COUNT(*) FILTER (WHERE billing_cycle = 'monthly'), 0),
        'yearly', COALESCE(COUNT(*) FILTER (WHERE billing_cycle = 'yearly'), 0)
      )
      FROM tenant_subscriptions
